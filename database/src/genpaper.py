...
def write_header(f,i,fom,to):
	...
	f.write('<p>从'+get_month_string(fom)+'到'+get_month_string(to)+'，对'+dmc[i]+'沉降情况进行了观测,对观测结果进行分析，结果如下：</p>\n')
	
	f.write('<h3 align="left">1．下沉数据统计</h3>\n')
	
	
def write_table(f,i,j,fom,to):
	...
	point=get_point(i,j)
	pmax,plst=get_data(i,j,point,fom,to)
...
def gen_paper(fom,to):
	f=open('./paper/1.html','w')
	write_header(f,'1',fom,to)
	write_table(f,'1','1',fom,to)
	write_table(f,'1','2',fom,to)
	write_analysis(f,'1')

	f=open('./paper/2.html','w')
	write_header(f,'2',fom,to)
	write_table(f,'2','1',fom,to)
	write_table(f,'2','2',fom,to)
	write_analysis(f,'2')

	f=open('./paper/3.html','w')
	write_header(f,'3',fom,to)
	write_table(f,'3','1',fom,to)
	write_table(f,'3','2',fom,to)
	write_table(f,'3','3',fom,to)
	write_table(f,'3','4',fom,to)
	write_analysis(f,'3')

	f=open('./paper/4.html','w')
	write_header(f,'4',fom,to)
	write_table(f,'4','1',fom,to)
	write_table(f,'4','2',fom,to)
	write_analysis(f,'4')
